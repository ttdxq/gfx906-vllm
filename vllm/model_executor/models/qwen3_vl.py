    ...
    def _get_video_second_idx(
        ...
        if do_sample_frames:
            ...
            if sampled_num_frames is not None:
                ...
                # Here sampled_fps is the fps of the sampled video, while
                # metadata["fps"] is the fps of the original video.
...
def _replace_video_token_placeholders(
    ...
    """Replace each video placeholder with its expanded token sequence."""
    result: list[int] = []
    repl_idx = 0
    i = 0
    ...
    while i < len(prompt_ids):
        if prompt_ids[i : i + target_len] == target:
    ...
    assert repl_idx == len(replacements), (
        f"Found {repl_idx} video placeholders but expected {len(replacements)}"
    )
    return result
...
class Qwen3VLMultiModalProcessor(BaseMultiModalProcessor[Qwen3VLProcessingInfo]):
    ...
    def _expands_only_video_token(hf_processor: ProcessorMixin) -> bool:
        """Return whether the HF processor expands only the bare video token."""
    ...
    def _call_hf_processor(
        ...
        if videos := mm_data.pop("videos", []):
            ...
            for item_idx, item in enumerate(videos):
                ...
                # Use our EVS-adjusted token sequence instead of HF input_ids.
    ...
    def _get_mm_fields_config(
        ...
        return dict(
            ...
            timestamps=MultiModalFieldConfig.batched("video"),
        )
